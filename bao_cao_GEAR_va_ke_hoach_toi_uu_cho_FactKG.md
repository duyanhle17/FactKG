...
GEAR sử dụng pipeline ba giai đoạn:

```text
Claim
  -> Document Retrieval
  -> Sentence Selection + Threshold Filter
...
Paper kế thừa phương pháp của Hanselowski et al. (2018):

1. Dùng constituency parser để trích xuất các entity tiềm năng trong claim.
2. Truy vấn MediaWiki API và giữ bảy kết quả xếp hạng cao nhất cho mỗi query.
3. Loại tài liệu không thuộc Wikipedia dump và lọc theo độ trùng từ giữa title và claim.

### 3.2. Sentence Selection

Sentence selector dùng biến thể ESIM để chấm relevance giữa claim và từng câu. Hệ thống gốc lấy top 5 câu; GEAR bổ sung threshold `τ` để loại evidence có điểm thấp. Vì vậy, số evidence đầu vào claim verifier không vượt quá 5.
...
Đây là phần đóng góp chính của paper, gồm sentence encoder, Evidence Reasoning Network và evidence aggregator.
...
Vector `[CLS]` cuối được dùng làm representation. Mỗi cặp evidence-claim được mã hóa riêng, nhưng evidence và claim vẫn được nối thành một BERT pair bên trong lần mã hóa đó.
...
Mỗi evidence là một node trong đồ thị fully-connected có self-loop. Trạng thái ban đầu:

```text
h_i^0 = e_i
```

Ở layer `t`, hệ số attention từ node `i` tới node `j` được tính bằng:

```text
p_ij = W1(ReLU(W0(h_i || h_j)))
α_ij = softmax_j(p_ij)
h_i^t = Σ_j α_ij h_j^(t-1)
```

ERNet cho phép evidence trao đổi thông tin trước khi tổng hợp. Đây là điểm phân biệt quan trọng giữa GEAR và một mô hình chỉ encode riêng rồi pooling.

### 4.3. Evidence Aggregator

Paper thử ba chiến lược:

- **Attention:** dùng claim representation để chấm trọng số evidence.
- **Max:** max pooling theo từng chiều.
- **Mean:** mean pooling theo từng chiều.
...
Sau đó, một classifier tạo phân phối trên ba nhãn từ vector tổng hợp `o`.

---

## 5. Kết quả thực nghiệm

### 5.1. Kết quả full pipeline
...
GEAR chứng minh rằng một pipeline dùng BERT đã fine-tune, graph reasoning và evidence aggregation có thể tận dụng multi-evidence tốt hơn các baseline trên FEVER. Paper cũng cho thấy retrieval vẫn là điểm nghẽn quan trọng và ERNet mang lại lợi ích trên tập claim khó cần nhiều evidence.
